...
class Client:
    ...
    def train_random_node(self, node: int, local_epochs: int):
        ...
        if self.current_global_epoch == 0:
            ...
            self.prox_model_state_dict = copy.deepcopy(model.model.state_dict())
            model.add_callback("on_train_epoch_start", self.start_prox_model)
            model.add_callback("on_train_epoch_end", self.prox_model)
            model.train(task="detect",
                        pretrained=False,
                        data=f'{self.cfg["yaml_files"]}/{node}.yaml',
                        epochs=local_epochs,
    ...
    def start_prox_model(self, trainer):
        trainer.prox_model = copy.deepcopy(self.prox_model_state_dict)

    def prox_model(self, trainer):
        trained_model = copy.deepcopy(trainer.model.to("cpu").model.state_dict())
        prox_model = copy.deepcopy(trainer.prox_model)
        temp_model = copy.deepcopy(trainer.prox_model)
        for t_key, p_key in zip(trained_model.keys(), prox_model.keys()):
            temp_model[p_key] = prox_model[p_key] - trained_model[t_key]
        for t_key, p_key in zip(trained_model.keys(), prox_model.keys()):
            trained_model[t_key] = trained_model[t_key] + torch.mul(temp_model[p_key], (0.01 / 2))
        trainer.model.model.load_state_dict(trained_model)
        trainer.model.to("cuda")
        del trained_model
        del temp_model
        trained_model = copy.deepcopy(trainer.ema.ema.to("cpu").model.state_dict())
        temp_model = copy.deepcopy(trainer.prox_model)
        ...
        for t_key, p_key in zip(trained_model.keys(), prox_model.keys()):
            trained_model[t_key] = trained_model[t_key] + torch.mul(temp_model[p_key], (0.01 / 2))
        trainer.ema.ema.model.load_state_dict(trained_model)
        trainer.ema.ema.to("cuda")
        del trained_model
        del temp_model
        del prox_model

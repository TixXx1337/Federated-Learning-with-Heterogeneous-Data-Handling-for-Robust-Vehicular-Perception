...
class Client:
    ...
    def train_random_node(self, node:int, local_epochs: int):
        ...
        if self.current_global_epoch == 0:
            ...
            prox_model = copy.deepcopy(model.model.state_dict())
            for epoch in range(local_epochs-1):
                model.train(task="detect",
                                        pretrained=False,
                                        data=f'{self.cfg["yaml_files"]}/{node}.yaml',
                                        epochs=1,
                                        batch=self.cfg["batch"],
                                        workers=self.cfg["workers"],
                                        name=f"{self.cfg['project']}/deleteme/GE{self.current_global_epoch}/{self.cfg['distribution']}{node}_LE{epoch}",
                                        val=False,
                                        warmup_epochs=0,
                                        device=self.cfg['devices']
                                        )
                del model
                temp_model = copy.deepcopy(prox_model)
                path = Path().resolve()
                model = YOLO(f"{path}/runs/detect/{self.cfg['project']}/deleteme/GE{self.current_global_epoch}/{self.cfg['distribution']}{node}_LE{epoch}/weights/last.pt")
                trained_model = copy.deepcopy(model.model.state_dict())
                for key in prox_model.keys():
                    temp_model[key] = prox_model[key] - trained_model[key]
                for key in prox_model.keys():
                    trained_model[key] = trained_model[key] + torch.mul(temp_model[key], (0.01/2))
                model.model.load_state_dict(trained_model)
                shutil.rmtree(f"{path}/runs/detect/{self.cfg['project']}/deleteme/GE{self.current_global_epoch}/{self.cfg['distribution']}{node}_LE{epoch}")
